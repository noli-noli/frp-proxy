...
# frp本体と.envを配置
if [ -f ../../frp/frps ] && [ -f ../../frp/frps.toml ]; then
    if [ -d "$FPR/frps" ]; then
        echo "[Check:WARN] Already exists ($FPR/frps) -> replace"
        rm -rf "$FPR/frps"
    fi
    mkdir -p "$FPR/frps"
    cp ../../frp/frps "$FPR/frps/"
    cp ../../frp/frps.toml "$FPR/frps/"
    echo "[Check:OK] Moved frps directory to $FPR"
    chown -R root:root "$FPR/frps"
    chmod -R go-rwx "$FPR/frps"
    echo "[Check:OK] Set owner root:root and permissions 700/600 ($FPR/frps)"
else
    echo "[Check:NG] Not found frps directory (../../frp/frps)"
    exit 1
fi
